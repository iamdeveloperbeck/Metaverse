...
const ExploreCard = ({ id, imgUrl, title, index, active, handleClick }) => (
  ...
  >
    ...
    {active !== id ? (
      <h3 className="font-boldd sm:text-[26px] text-[18px] text-white absolute z-0 lg:bottom-20 lg:rotate-[-90deg] lg:origin-[0,0]">
        {title}
      </h3>
    ) : (
      <div className="absolute bottom-0 p-8 justify-start w-full flex-col bg-[rgba(0,0,0,0.5)] rounded-b-[24px]">
        ...
        <h2 className="font-semibold mt-[24px] sm:text-[32px] text-[24px] text-white">{title}</h2>
      </div>
    )}
  </motion.div>
);
